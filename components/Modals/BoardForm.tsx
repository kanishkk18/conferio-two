    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (formIsValid) {
            if (props.formType === 'new') {
                ...
                const columns: Array<{ uuid?: string; name: string; position: number }> = [];
                if (columnsInput.value) {
                    columnsInput.value.forEach((column, i) => {
        ...
        <>
            <div className={`flex-col  ${deleteColumnsModal.isOpen ? 'hidden' : 'flex'}`}>
                ...
                <form ref={formRef} onSubmit={handleSubmit} action="submit" className="flex flex-col">
                    ...
                    /> */}
                    <Button variant="default" data-testid="board-submit">
                        {props.formType === 'new' ? 'Create Board' : 'Save Changes'}
                    </Button>
                </form>
            </div>
            <DeleteTaskModal />
