  ...
  return (
    <div className="relative p-4 lg:p-5 group before:absolute before:inset-y-8 before:right-0 before:w-px before:bg-gradient-to-b before:from-input/30 before:via-input before:to-input/30 last:before:hidden">
      <div className="relative flex items-center gap-4">
        ...
        />
        {/* Icon */}
        <div className="max-[480px]:hidden size-10 shrink-0 rounded-full bg-[linear-gradient(220.55deg,#3793FF_0%,#0017E4_100%)] border border-blue-700/50 flex items-center justify-center text-white">
          {icon}
        </div>
        {/* Content */}
        <div>
...
export function StatsGrid({ stats }: StatsGridProps) {
  return (
    <div className="grid grid-cols-2 min-[1200px]:grid-cols-4 border border-border dark:border-neutral-700 rounded-xl bg-gradient-to-br from-sidebar/60 to-sidebar dark:from-[#1E1E21]/60 dark:to-[#1E1E21]">
      {stats.map((stat) => (
