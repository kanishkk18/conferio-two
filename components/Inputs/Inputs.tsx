...
const ErrorMsg: FC<PropsWithChildren> = ({ children }) => {
    return (
        <span className="absolute right-3 -top-2.5 whitespace-nowrap px-1 text-sm text-danger  sm:top-2.5 sm:right-4">
            {children}
        </span>
    );
};
    ...
    return (
        <div className="relative w-full">
            <input
                ...
                } h-10 min-h-fit w-full cursor-pointer rounded border-2 py-2 px-4 text-sm font-medium text-black placeholder-black placeholder-opacity-25 outline-none focus:placeholder-opacity-0 dark:text-white dark:placeholder-white dark:placeholder-opacity-25 ${
                    className ?? ''
                }`}
            />
        ...
        <div className="relative w-full">
            <textarea
                ...
                className={`${
                    ...
                    small ? 'h-10' : 'h-28'
                } w-full cursor-pointer rounded border-2 py-2 px-4 text-sm font-medium text-black placeholder-black placeholder-opacity-25 outline-none focus:placeholder-opacity-0 dark:text-white dark:placeholder-white dark:placeholder-opacity-25 ${
                    className ?? ''
                }`}
            />
            {haserror && errorMsg && <ErrorMsg>{errorMsg}</ErrorMsg>}
        ...
        <fieldset className={`flex flex-col text-mid-grey dark:text-white ${className ?? ''}`}>
            ...
            <div className="relative">
                <select
                    ...
                    value={value}
                    {...restProps}
                    className="h-10 w-full cursor-pointer appearance-none rounded border-2 border-mid-grey border-opacity-25 py-2 px-4 text-sm font-medium text-black placeholder-black placeholder-opacity-25 outline-none hover:border-primary focus:border-primary focus:placeholder-opacity-0 dark:text-white dark:text-inherit dark:placeholder-white dark:placeholder-opacity-25"
                >
                ...
                <PopoverEl anchorWidth={true}>
                    <div className="absolute top-12 w-full rounded-md p-4 bg-white dark:bg-neutral-800">
    ...
    return (
        <div className={props.className ?? ''}>
            ...
            >
                ...
                <Button id={props.id ? `${props.id}-add` : undefined} type="button" onClick={onNewColumn} className='text-white' >
                    {props.addBtnText}
                </Button>
            </DndContext>
        </div>
    ...
    ({ checked, id, ...props }, ref) => {
        ...
        return (
            <>
                ...
                <label
                    htmlFor={id}
                    className={`relative flex aspect-square min-w-[1rem] cursor-pointer items-center justify-center overflow-hidden rounded border border-mid-grey border-opacity-25 transition-all dark:bg-red-500 ${props.className}`}
                >
                    <div
                        className={`absolute z-0 h-full w-full -translate-x-2 translate-y-2 rounded-full  transition-all ${
